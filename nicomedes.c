...
// requires the -lm option for the C compiler on Linux systems so it can find the math header and library files.
...
*/
    ...
    {  
        printf("Error: ");
        if (error==NONE)            writeln("NONE");            else
        if (error==BAD_DATA)        writeln("BAD_DATA");        else
        if (error==NO_CONVERGENCE)  writeln("NO_CONVERGENCE");  else
        printf("%d" EOL, error);
    }
    return result;
}
...
int main(int argc, char** argv)
{
    ...
    "parameter: a positive integer specifying the number of decimal "	EOL
    "digits of accuracy. The default is 6."      	   				 EOL;
    ...
    {
        printf("Specified accuracy (%d) out of range (0..20). "
			    "Using default of 6." EOL, accuracy);
        accuracy = 6;
    }
    ...
    if ((x<=0) || (y<=0))
    {
        writeln( "Error: point must be in first quadrant, "
